...
- **B2-1.** After login the app reads `account_type` and renders the matching **3-tab bottom navigation**, tabs always visible, each **icon + label**:
  - Customer → **Home · AI Agent · Account**
  - Partner → **Home · Job Board · Account**
- **B2-2.** The bottom tab bar is the only navigation. No hamburger, no hidden gestures, no horizontal swiping; back is predictable.
- **B2-3.** Both Home screens carry the **global "partners onboarded" counter** (§4.8) as a top strip.

### 4.2 Customer — Home (map)

- **B2-4.** Home is a **full-screen, Google-Maps-style map**, centered on the **customer's current location** (device GPS) at a sensible default zoom.
- **B2-5.** The map shows **partner pins** whose last-known location falls **within the current map viewport**: **Ride Partners (🛺)** and **Delivery Partners (🛵)** with distinct markers.
- **B2-6.** **Viewport-driven refresh:** when the customer pans or zooms, the app re-queries partners for the **newly visible region** once the map settles (debounced ~400 ms). This is the **only** refresh trigger — no polling, no interval, no auto-movement of pins.
- **B2-7.** Partner positions are **static last-known** locations (written when each partner last opened the app, §4.9). They do **not** move in real time; a pin is wherever that partner last opened Qarmo.
- **B2-8.** Partners appear as **plain type icons** on the map (🛺 Ride / 🛵 Delivery) — **no tap callout**. No name, phone, address, or status is shown; the icon alone signals that a partner of that type is there.
- **B2-9.** **Result cap:** a single query returns at most **100** partners; if the region holds more, show the 100 nearest to the map center and a subtle "Zoom in to see more" hint.
- **B2-10.** The customer's own location is used **only to center the map**. It is **never written to the database and never shown to partners**.
- **B2-11.** **Location denied:** center the map on **Kochi** (the default fallback coordinates), show a one-line fix-it banner ("Turn on location to see partners near you. **Open settings**"), and still allow panning/querying.

### 4.3 Customer — AI Agent

- **B2-12.** A centered **"Coming soon"** placeholder (icon + words), no other controls or content.

### 4.4 Customer — Account

- **B2-13.** Shows the customer's profile: photo (if set), **name**, **phone**, and **account type**. **View-only.**
- **B2-14.** A **Log out** action (secondary style) with a confirm: "Log out? **Yes / No**".
- **B2-15.** Editing profile details is **out of scope** for the beta.

### 4.5 Partner — Home (ID card)

- **B2-16.** Home shows an **ID-card-style summary**: partner **photo**, **name**, **partner type** (🛺 Ride / 🛵 Delivery), **city**, and **plate number** — laid out like an identity card.
- **B2-17.** The card leads with **referral figures**:
...
- **B2-18.** The card shows the partner's **own shareable referral code**, with a **Share my code** action — the single amber element on this screen — that opens the OS share sheet pre-filled with **"Join Qarmo with my code &lt;CODE&gt;"** (e.g., "Join Qarmo with my code QARM123").
- **B2-19.** **Earnings are hidden** in the beta (no jobs yet). The layout reserves room so an earnings figure can join the card once the Job Board ships — but nothing about earnings is shown now.

### 4.6 Partner — Job Board

- **B2-20.** A centered **"Coming soon"** placeholder (icon + words), no job list, no other controls.

### 4.7 Partner — Account

- **B2-21.** Shows the partner's profile: photo, **name**, **phone**, **partner type**, **city**, **plate number**. **View-only.**
- **B2-22.** A **Log out** action with the same confirm as B2-14. Editing is out of scope.

### 4.8 "Partners onboarded" counter (shared)

- **B2-23.** Both Home screens show a **global count of partners onboarded app-wide** ("512 partners on Qarmo") — the same number for everyone. It **updates live** via a Supabase Realtime subscription: the initial value is fetched on Home open, then the number ticks up on its own as new partners join while the screen is visible.
- **B2-24.** The global number counts `profiles` where `account_type = 'partner'`.
- **B2-25.** On the **partner** Home only, the ID card **additionally** shows the partner's **personal** referred-count (people they onboarded) — distinct from the global number (B2-17).
- **B2-26.** Copy handles singular/zero gracefully ("1 partner on Qarmo", "Be the first — invite a friend").

### 4.9 Partner location capture (on app open)

- **B2-27.** When a **partner** opens the app (cold start or foreground), capture the device's current GPS **once** and write it to `last_location` + `location_updated_at` on their profile. **One-shot only** — no interval, no background service, no continuous tracking.
- **B2-28.** Requires location permission. If denied, the partner is simply **not placed on the map** (no block on using the app); show a gentle one-line note: "Turn on location so customers can find you."
- **B2-29.** **Customers never write location** — only partners do. A partner's stored location is exposed to customers **only** through the bounded map query (§6.2), returning coarse fields.
...
- [ ] Post-login routing shows the correct 3 tabs per `account_type`
- [ ] Customer Home map centers on GPS; partner pins (🛺/🛵) render for the current viewport
- [ ] Panning/zooming re-queries the new region (debounced); no interval/polling exists
- [ ] Partner pins are static last-known positions drawn as type icons; no tap callout
- [ ] Result cap (100) + "zoom in" hint verified in a dense region
- [ ] Customer location denial falls back to the **Kochi** center with a fix-it banner; customer location never written
- [ ] Partner writes `last_location` once on app open; denial leaves them off-map without blocking
- [ ] `partners_in_bounds` RPC is the only customer path to partner locations; direct RLS exposes nothing
- [ ] Global "partners onboarded" counter shows on both Homes and updates **live** (Realtime) as partners join
- [ ] Partner ID card shows points (referred×50, green), "N friends joined", code, and a working Share; earnings hidden
- [ ] AI Agent and Job Board show only "Coming soon"; Account is view-only with a confirmed Log out
- [ ] Every screen passes the Design Philosophy §8 checklist on a budget Android device; all copy via i18n keys
